...
class CRN(nn.Module):
    def __init__(self, n_top=7, kernel1=3, kernel2=3, channel1=56, channel2=12, channel3=64, nonlinear="PReLU"):
        ...
        self.common_network = self.generate_commonnetwork().cuda()
        self.subnetwork_list = [self.generate_subnetwork().cuda() for _ in range(n_top+1)]

        self._initialize_weights()

    def forward(self, x):
        features = self.common_network(x).cuda()
        # print("common")
        components = [sub_network(features).cuda() for sub_network in self.subnetwork_list]
        # print("sub")
        return torch.stack(components, dim=1).cuda()    # (N, n_top, C, H, W)
